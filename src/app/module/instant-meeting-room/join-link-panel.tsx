import React, { useState } from 'react';
import { Box, Button, Image, Flex, Text } from '@chakra-ui/react';
import AddUserButton from '../common/add-user-button';
import { useMeetingContext } from '@/context/meetingContext';
import QRCodeGenerator from './qr-code-generator';
import URLClipboard from './url-clipboard';

interface JoinLinkPanelProps {
  closeThePanel: () => void;
}

function JoinLinkPanel({ closeThePanel }: JoinLinkPanelProps) {
  const { meeting } = useMeetingContext();
  const [copyMessage, setCopyMessage] = useState('');
  const handleCopy = () => {
    setCopyMessage('URL copied successfully!');
    setTimeout(() => setCopyMessage(''), 2000);
  };

  const meetingURL = meeting?.meetingURL;
  return (
    <Box
      zIndex="1"
      w="340px"
      h="396px"
      borderRadius="20px"
      boxShadow="0 0 16px 0 rgba(0, 0, 0, 0.08)"
      position="absolute"
      bgColor="#fff"
      top={{ base: '250px', md: '370px' }}
      display="flex"
      flexDirection="column"
      alignItems="flex-start"
      px="24px"
      py="16px"
      gap="1vw"
    >
      <Flex w="100%" h="10%" justifyContent="space-between" alignItems="center">
        <Text fontWeight="bold" fontSize="md">
          Your meeting&apos;s ready
        </Text>
        <Button
          onClick={closeThePanel}
          bgColor="#fff"
          borderRadius="50%"
          p={0}
          minW="auto"
          h="24px"
          _hover={{ bg: 'gray.100' }}
        >
          <Image src="/join-link-panel/close.svg" w="16px" h="16px" />
        </Button>
      </Flex>
      <AddUserButton backgroundColor="#046ffb" information="Add others" />
      <Text fontSize="14px">Or scan the QR code to join the meeting</Text>
      {meetingURL && <QRCodeGenerator url={meetingURL} />}
      <Text fontSize="14px">Or share this meeting link with others you want in the meeting</Text>
      <Box
        bgColor="#E5F0FE"
        w="100%"
        h="10%"
        borderRadius="18px"
        display="flex"
        justifyContent="center"
        alignItems="center"
      >
        {meetingURL && (
          <URLClipboard handleCopy={handleCopy} url={meetingURL} copyMessage={copyMessage} />
        )}
      </Box>
    </Box>
  );
}

export default JoinLinkPanel;
